...
Build target: FreeSnitch. Hit ⌘R. The first run will fail to install the helper daemon if you're not signed with a Developer ID — that's expected for dev builds. The GUI still launches and is fully interactive against in-memory state.
...
- [ ] Builds clean with `xcodegen generate && xcodebuild -project FreeSnitch.xcodeproj -scheme FreeSnitch`
...
- macOS version
- FreeSnitch version (Settings → About)
- Reproduction steps
