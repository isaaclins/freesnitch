...
- `FilterDataProvider.swift`: evaluates every new socket flow against the rule
  set, returns allow or drop, or **pauses** the flow and asks the GUI, resuming
  with the user's verdict. Reuses `RuleMatcher` from `Shared/`.
- IPC: `Shared/IPCConnection.swift` (app and extension XPC, modelled on Apple's
  `SimpleFirewall` sample).
- Rules: `Shared/SharedRuleBridge.swift`: the GUI mirrors the active rules and
  mode into the app-group container; the extension reads them (the extension is
  sandboxed and cannot read the helper DB directly).
...
1. Enable the **Network Extensions** capability on the App ID in the Apple
   Developer portal (self-serve since 2016, with no email request needed) and add
...
2. Two Developer ID provisioning profiles (app + extension) carrying that
   capability.
3. Sign with **Developer ID Application** from the inside out: helper, then
   extension, then app. **Notarize** the result; `Scripts/release.sh` performs
